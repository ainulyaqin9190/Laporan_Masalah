...
<div class="bg-white rounded shadow p-4">
  ...
  <div class="mb-2 flex items-center gap-2">
    <span class="font-semibold">Nomor Tiket:</span>
    <span id="nomorTiket" class="font-mono">{{ $laporan->nomor_laporan }}</span>

    <button 
      class="px-2 py-1 text-sm rounded bg-blue-500 text-white hover:bg-blue-600"
      onclick="salinNomorTiket()">
      Salin
    </button>
  </div>
  ...
  <div class="mb-2">
    ...
    <span class="px-2 py-1 rounded bg-gray-100 border">{{ ucfirst($laporan->status) }}</span>
  </div>
...
<script>
function salinNomorTiket() {
  const nomor = document.getElementById('nomorTiket').textContent;
